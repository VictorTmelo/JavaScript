class Linalg {
    ...
    potencia(a){
        ...
        var ax0 = this.multi(a,chute);

        var maior = this.pegaMaior(ax0)

        var x = this.mul(1/maior, ax0);

        var ax = this.multi(a,x);

        var lambida = this.div(this.multi(this.trans(ax),x) , this.multi(this.trans(x),x));


        while(Math.abs((lambida.get(1,1) - lambidaAnt) / lambida.get(1,1)) > 1e-5 ){

            maior = this.pegaMaior(ax)

             x = this.mul(1/maior, ax);

             ax = this.multi(a,x);

             lambidaAnt = lambida.get(1,1)

             lambida = this.div(this.multi(this.trans(ax),x) , this.multi(this.trans(x),x))

        }

        var autoVetor = x;

        var autoValor  = Math.round(lambida.get(1,1))
    ...
    deflacao(a){
        ...
        var autoValor = this.potencia(a)[0];

        var autoVetor = this.potencia(a)[1];

        var mul = this.multi(autoVetor, this.trans(autoVetor))

        var mul2 = this.multi(this.trans(autoVetor), autoVetor)

        var div = autoValor / mul2.get(1,1);

        var mul3 = this.mul(div, mul)

        var b = this.sub(a,mul3)
